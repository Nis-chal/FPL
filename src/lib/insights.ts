...
export async function getLeagueInsights(horizon = 5, includeAccumulated = true) {
  ...
  // Captain = best single-GW expected points among likely starters
  const captainPool = [...scored]
    .filter((p) => p.startChance >= 0.5 && p.availabilityFactor >= 0.5)
    .sort((a, b) => b.expectedPointsPerGw - a.expectedPointsPerGw);
  const captainPick = captainPool[0] ?? topScorers[0] ?? bestSquad.captain;
...
export async function getPlayerDetail(playerId: number, horizon = 5) {
  ...
  const player = scored.find((p) => p.id === playerId);
